...
exports.newUtil = async function($, deploymentSpec, templates) {
    ...
    that.createRedisDedicatedDisk = async function(redisProps) {
        if (redisProps.isDedicatedVolume) {
            ...
            /* WARNING: `redisProps.timestamp` needs to match the
            ...
            */
            const deleteR = await k8sClient.apis.apps.v1
                .namespaces(redisSpec.k8SNamespace)
                .deployments('redis-' + redisProps.id);
            ...
            const createR = await k8sClient.apis.apps.v1
                .namespaces(redisSpec.k8SNamespace)
                .deployments.post({body: r[1]});


            $._ && $._.$.log && $._.$.log.debug('Disk created: ' + createD);
            return [createD, deleteR, createR];
        } else {
            throw new Error('Creating disk with invalid properties');
        }
    };

    that.updateRedis = async function(redisProps, redisPatch) {
        return k8sClient.apis.apps.v1.namespaces(
            appSpec.k8SNamespace
