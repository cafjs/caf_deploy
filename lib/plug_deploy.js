...
exports.newInstance = function(context, spec, secrets, cb) {
    ...
    var map = spec.env.map || process.env.CF_MAP;
    ...
    that.deployApp = function(caId, name, fileName) {
        var dir;
        if (name.indexOf(caId.replace(/_/g,'-') + '-') === 0) {
            async.waterfall([
                                function(cb0) {
                                  //unzip, untar
                                ...
                                function(isUp, cb0) {
                                     // update or deploy
                                    if (isUp) {
                                        $.log && $.log.trace("deploy:deployApp:"
                                                             +" Update caId=" 
                                                             + caId + " name=" 
                                                             + name + " file="
                                                             + fileName);
                                        helper.updateApp($, vmc, name, dir,
                                                         cb0);
                                    } else {
                            ...
                            function(err, data) {
                                if (err) {
                                    ...
                                    $.log.debug(msg + JSON.stringify(err));
                                }
                                $.log && $.log.trace("deploy:deployApp:"
                                                     +" Done caId=" 
                                                     + caId + " name=" 
    ...
    that.deleteApp = function(caId, name) {
        if (name.indexOf(caId.replace(/_/g,'-') + '-') === 0) {
            $.log && $.log.trace("deploy:deleteApp: caId=" 
                                 + caId + " name=" + name);
        ...
        vmc.apps(function(err, apps) {
                     if (err) {
                         ...
                         for (var i = 0; i < apps.length; i++) {
                             ...
                             if (appName.indexOf(caId.replace(/_/g,'-') +
                                                 '-') === 0) {
                         ...
                         $.log && $.log.trace("deploy:refreshInfo: caId=" 
                                              + caId + " result=" +
