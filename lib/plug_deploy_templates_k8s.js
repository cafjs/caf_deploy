...
/**
 * @global
 * @typedef {Object} flexPatchesType
 * @property {Object|null} redis The redis service patch.
 * @property {Object|null} app The app process patch.
 */
...
exports.newTemplates = function($, deploymentSpec) {
    ...
    /**
     *
     * @param {flexOptionsType} flexOptions
     * @param {boolean} upgrading
     *
     * @return {patchedPropsType}
     *
     */
    const computeResources = function(flexOptions, upgrading) {
        const {plan, numberOfCAs, currentProps} = flexOptions;
        ...
        if (upgrading) {
            ...
            const instances = computeInstances(numberOfCAs, plan);
            const {cpus, memory, memoryLimit, cpusLimit, dedicatedVolumeSize} =
                  verticalRedisScaling(instances);
    ...
    /**
     * Update the property `app.props` that summarizes the new deployment
     * metadata.
     *
     * @param {deploymentPropsType} currentProps
     * @param {patchedPropsType} newProps
     ...
     * @return {patchedPropsType}
     */
    const updateAppProps = function(currentProps, newProps) {
        const updateImpl = (target, source, props) => {
            props && props.forEach((x) => {
        ...
        if (newProps) {
            const old = myUtils.deepClone(currentProps);
            old.plan = newProps.plan;
            old.numberOfCAs = newProps.numberOfCAs;
            updateImpl(old.redis, newProps.redis, [
                'cpus', 'memory', 'memoryLimit', 'cpusLimit', 'poolKey',
                'poolValue', 'isGvisor', 'dedicatedVolumeSize'
            ]);
            updateImpl(old.app, newProps.app, [
                'cpus', 'memory', 'memoryLimit', 'cpusLimit',
                'storage', 'storageLimit', 'egressLimit',
                'poolKey', 'poolValue', 'isGvisor', 'isIncubator', 'instances'
            ]);
            newProps.app.props = JSON.stringify(old);
        }
        return newProps;
    };
    ...
    /**
     *
     * @param {patchedPropsType} deltaProps
     *
     * @return {flexPatchesType}
     */
    const genPatches = function(deltaProps) {
        const genPatchesRedis = (redis) =>
              redis ?
              {
              } :
              null;

        const genPatchesApp = (app) =>
        // need to patch env prop for MEMORY for `args`
              app ?
              {
                  spec: {
        ...
        return {
            redis: genPatchesRedis(deltaProps.redis),
            app: genPatchesApp(deltaProps.app)
        };
    };
     ...
     *
     *
     * @param {flexOptionsType} flexOptions The arguments to create
     * patches.
     *
     * @return {flexPatchesType|null} The patch to flex resources or null if no
     * change needed.
     */
    that.createPatches = function(flexOptions) {
         ...
         *  Four steps:
         *
         * 1. Compute new resources     (flexOptionsType->patchedPropsType)
         * 2. Find delta with existing ones (patchedPropsType->patchedPropsType)
         * 2. Update `app.props` (patchedPropsType->patchedPropsType)
         * 4. Generate patches if needed (patchedPropsType->flexPatchesType)
         */
        const currentProps = flexOptions.currentProps;
        const appProps = currentProps.app;
        const upgrading = appProps.isIncubator &&
            isIncubatorUpgrade(flexOptions);
        const newProps = computeResources(flexOptions, upgrading);
        const deltaProps = deltaResources(currentProps, newProps);
        const updatedProps = updateAppProps(currentProps, deltaProps);
        return updatedProps ?
            genPatches(updatedProps) :
            null;
    };
