...
exports.updateApp = function($, vmc, appName, appDir, cb) {
    async.series([
                     function(cb0) {
                         // stackato hangs in update if not previously stopped
                         vmc.stop(appName, cb0);
                     },
                     function(cb0) {
                         vmc.update(appName, appDir, cb0);
                     }
                 ], safeF(cb));
};

exports.deployApp = function($, vmc, appName, appDir, manifest, services, map,
                             cb) {
    var replaceHost = function(mapUrl, newName) {
        var parsedTarget = url.parse(mapUrl);
        parsedTarget.host = parsedTarget.host.replace(DEFAULT_MAP_PREFIX,
                                                      newName);
        var result = url.format(parsedTarget);
    ...
    async.series([
                     ...
                     function(cb0) {
                         if (map) {
                             vmc.map(appName, replaceHost(map, appName), cb0);
                         } else {
                             cb0(null);
                         }
                     }
