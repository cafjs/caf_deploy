 ...
 * @property {Object} redis The redis service patch.
 * @property {Object} app The app process patch.
 */
...
exports.newTemplates = function($, deploymentSpec) {
    ...
    that.createPatches = function(flexOptions) {
        const computePatches = function(newProps) {
            //app
            const replica = {
                spec: {
        ...
        const appProps = flexOptions.currentProps.app;
        const redisProps = flexOptions.currentProps.redis;
        if (appProps.isIncubator) {
            if (isIncubatorUpgrade(flexOptions)) {
                const newProps = computeUpgradedResources(flexOptions);
                return generatePatches(flexOptions, newProps);
            } else {
                return null;
            }
        } else {
            const newProps = computeResources(flexOptions);
            if (changedResources(newProps, flexOptions)) {
                return generatePatches(flexOptions, newProps);
            } else {
                return null;
            }
